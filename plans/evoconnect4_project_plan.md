...
- **Hall-of-fame opponents**: keep a roster of past strong agents (even after "death") that current agents occasionally face — a natural extension of Phase 9's fixed-opponent fitness-counting pattern, using the same `game_type` and stats-update mechanic, once fixed-opponent (heuristic) pressure alone is proven to work.
