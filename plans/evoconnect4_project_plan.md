...
| 9 — Heuristic challenge | Every agent plays `heuristic_games_per_agent_per_tick` games against the fixed heuristic bot each tick (deterministic, not sampled). A two-track fitness formula weights heuristic performance at β=0.7 independently of peer-game volume. Heuristic losses earn survival credit proportional to game length (α=0.5), creating a fitness gradient from tick 1 before any agent can win. New `game_type='evolution_heuristic'`; seven new agent counter columns for the split fitness tracks. Setting `heuristic_games_per_agent_per_tick=0` restores Phase 8 behavior exactly. See `plans/phase-9-heuristic-challenge.md` | Average game length in `evolution_heuristic` games trends upward and heuristic win rate in `benchmark_results` shows measurable improvement over a multi-thousand-tick run; disable param reproduces Phase 8 behavior identically |
| 10 — Tactical heuristic, adaptive weights, Hall of Fame | Three interlocking fixes for Phase 9's saturation problem (heuristic win rate hits 1.0 by tick 500, agents never learn defensive blocking). (1) `tactical_heuristic_bot` adds fork creation and fork blocking to the heuristic's priority order, restoring the fitness gradient. (2) Adaptive β_h and β_hof computed from a rolling benchmark window automatically shift weight from the heuristic track to the HoF track as mastery increases. (3) A Hall of Fame keeps past-best agents as a standing challenge pool; `_run_hof_maintenance()` (every 50 ticks) runs unified entry and eviction; per-tick HoF challenge games contribute a third fitness track. Four new agent counter columns; `hall_of_fame` DB table; 14 new config params (all `MUTABLE_FIELDS`). See `plans/phase-10-tactical-heuristic-hof.md` | Benchmark heuristic win rate starts lower than Phase 9 baseline (tactical bot is harder) and trends upward over thousands of ticks; adaptive weights shift observably as win rate changes; HoF members accumulate over maintenance events and living agents show rising `hof_fitness` trend |
...
- **Hall-of-fame opponents**: implemented in Phase 10. Natural next extension: separate per-bot-level HoF tiers (e.g., one hall per heuristic bot level) once `heuristic_bot_level` escalates beyond `"tactical"`.
